function sos = my_sosineq(sos,symexpr,new_decvartable,info,info2)
...
if isfield(sos,'symvartable')
	...
	if nargin > 3 & isnumeric(info)
        ...
        for i = 1:size(symexpr,1)
            ...
            sos = my_sosconstr(sos,'ineq',symexpr(i),new_decvartable);
...
% Sparse polynomial
if nargin > 3 & strcmp(info,'sparse');
   for i = 1:size(symexpr,1)
       sos = my_sosconstr(sos,'ineq',symexpr(i),new_decvartable);
...
% Sparse polynomial
if nargin > 3 & strcmp(info,'sparsemultipartite');
   for i = 1:size(symexpr,1)
       sos = my_sosconstr(sos,'ineq',symexpr(i),new_decvartable);
...
sos = my_sosconstr(sos,'ineq',symexpr,new_decvartable);
