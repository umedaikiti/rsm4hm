function [At,b,Z] = my_getequation(symexpr,vartable,decvartable,varmat,new_decvartable)
...
if isa(symexpr,'polynomial')
    ...
    for i = 1:dimp
        for j = i:dimp
            ...
            coefmatr = double(subs(g,new_decvartable,zeros(numel(new_decvartable),1)));
...
%    new_decvartable_zeros = zeros(numel(new_decvartable),1);
%    new_decvartable_zerosf = zeros(1,length(new_decvartable));
     

    coefmons = arrayfun(@(x) feval(symengine,'poly2list',x,charvartable), FPexpr,'UniformOutput',false);

    coefmonmatrs = arrayfun(@(x) subs(x{:}.',new_decvartable,zeros(numel(new_decvartable),1)),coefmons,'UniformOutput',false);
...
%                g = matlabFunction(arrayfun(@(x) x(2:end), coefmon.'), 'Vars', {new_decvartable});
                coefmonmatr = coefmonmatrs(i,j); %g(zeros(1,length(new_decvartable)));
                coefmonmatr = coefmonmatr{:};
                Z = double(coefmonmatr(:,2:end));

%                coefmonmatr = subs(arrayfun(@(x) x(2:end), coefmon.'),new_decvartable,new_decvartable_zeros);
%                Z = double(coefmonmatr);

%                coefmonmatr = subs(coefmon.',new_decvartable,new_decvartable_zeros);
%                Z = double(coefmonmatr(:,2:end));
        ...
        for i = 1:dimp
            for j = i:dimp
                coefmon = coefmons(i,j); %feval(symengine,'poly2list',FPexpr(i,j),charvartable);
                coefmon = coefmon{:};
                
%                g = matlabFunction(coefmon.', 'Vars', {symvar(new_decvartable)});
                coefmonmatr = coefmonmatrs(i,j); %g(zeros(1,length(new_decvartable)));
                coefmonmatr = coefmonmatr{:};
